...
class Storage implements IStorage
{
   ...
   * Saját kiegészítés
   */
  public function addCard($id, $card)
  {
    $this->contents[$id]["cards"][] = $card;
  }
  /**
   * Saját kiegészítés
   */
  public function removeCard($id, $card)
  {
    $array = [];
    foreach ($this->contents[$id]["cards"] as $elem) {
      if ($elem !== $card) $array[] = $elem;
    }
    $this->contents[$id]["cards"] = $array;
  }
  ...
  public function decrMoney($id, $value)
  {
  ...
  public function incrMoney($id, $value)
  {
    $this->contents[$id]["money"] += $value;
  }
  ...
  public function changeId($old_id, $new_id)
  {
    $this->contents[$old_id]["id"] = $new_id;
    $this->contents[$new_id] =  $this->contents[$old_id];
    unset($this->contents[$old_id]);
  }
   ...
   */
   ...
   public function changeCard($id,$name, $type,$hp,$attack,$defense,$price,$description,$image)
   {
    $this->contents[$id]["name"] = $name;
    $this->contents[$id]["type"] = $type;
    $this->contents[$id]["hp"] = $hp;
    $this->contents[$id]["attack"] = $attack;
    $this->contents[$id]["defense"] = $defense;
    $this->contents[$id]["price"] = $price;
    $this->contents[$id]["description"] = $description;
    $this->contents[$id]["image"] = $image;
   }
